...
function readmeContent(project: AgentFrontendProject, packageJson: ScaffoldPackageJson): string {
  ...
  const lines = [
    ...
    "> This is a Vite + React + TypeScript project with the SDK vendored under `vendor/`. Run",
    "> `npm install` then `npm run dev` and give me the local URL — do NOT open index.html via",
    "> file:// (ES modules are blocked there and the page goes blank). I should see a full Agent",
    "> UI (sidebar, conversation, tool-call cards, reasoning, output panel) with a demo event",
    "> stream replaying automatically.",
    "",
    ...
    "```",
    "",
    "1. Implement `toStandardEvents(raw)` for your backend (Claude, Codex, OpenAI, LangGraph, or",
    "   your own). Return `[]` for keep-alives.",
    ...
    "1. 为你的后端实现 `toStandardEvents(raw)`(Claude / Codex / OpenAI / LangGraph / 自有均可);",
